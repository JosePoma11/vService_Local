...
import { emailBusiness, passBusiness } from '../config/_vGlobal.js';
import { activeAccount, recoveryPassword } from '../models/usuarios/designEmail/activeAccount.js';
...
router.post('/send-cod-reset-password', async (req, res) => {
  try {
    ...
    if (usuarioEncontrado) {
      ...
      await enviarCorreo(usuarioEncontrado.email, recoveryPassword(codigo), 'recover');
...
router.get('/resend-code/:id', async (req, res) => {
  ...
  try {
    ...
    if (!userCode) {
      ...
      await enviarCorreo(usuario.email, activeAccount(userCode.codigo), 'active');
    }
