...
// Ruta para actualizar la propiedad "location" de una factura a false
router.put('/update-location/:id', async (req, res) => {
  try {
    const facturaId = req.params.id;
    const newLocation = req.body.location; // Recibe el valor 'location' del cuerpo de la solicitud

    // Verificar si la factura con el ID proporcionado existe
    const factura = await Factura.findById(facturaId);

    if (!factura) {
      return res.status(404).json({ mensaje: 'Cambio de Ubicacion: Orden no encontrada' });
    }

    // Actualizar la propiedad "location" con el nuevo valor numérico
    factura.location = newLocation;
    if (newLocation === 3) {
      factura.estadoPrenda = 'donado';
    }
    await factura.save();

    // Devolver el ID, codRecibo, Nombre y location de la factura actualizada
    const facturaActualizada = {
      onAction: 'updateLocation',
      _id: factura._id,
      location: factura.location,
    ...
    console.error(error);
    res.status(500).json({ mensaje: 'No se pudo actualizar la ubicacion de orden de servicio' });
  }
});
