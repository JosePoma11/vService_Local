...
export const openingHours = async (req, res, next) => {
  ...
  if (!rol) {
    return res.status(400).json({ error: 'Falta enviar el rol de esta cuenta' });
  }
  ...
  if (estado) {
    ...
    //   const currentHour = moment(); // Obtén la hora actual como objeto Moment

    //   if (dias.includes(currentDay)) {
    //     const startTime = moment(horas.inicio, 'HH:mm').subtract(1, 'hour');
    //     const endTime = moment(horas.fin, 'HH:mm').add(1, 'hour');

    //     if (currentHour.isBetween(startTime, endTime)) {
    //       // Si el día y la hora son válidos, permite continuar
    //       next();
    //       return;
    //     } else {
